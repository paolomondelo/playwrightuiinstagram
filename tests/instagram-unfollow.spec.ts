...
import { ACCOUNTS_TO_UNFOLLOW } from '../data/accounts-to-unfollow';
...
test.describe('Instagram unfollow batch', () => {
  test.beforeEach(() => {
    test.skip(!username || !password, 'Set IG_USERNAME and IG_PASSWORD in .env before running');
  });

  test('unfollow listed accounts that do not follow back', async ({ page }) => {
    // ~177 accounts × ~5–8s each — allow up to 60 minutes
    ...
    console.log(`\nUnfollowing ${ACCOUNTS_TO_UNFOLLOW.length} accounts...\n`);

    for (let i = 0; i < ACCOUNTS_TO_UNFOLLOW.length; i++) {
      const handle = ACCOUNTS_TO_UNFOLLOW[i];
      const progress = `[${i + 1}/${ACCOUNTS_TO_UNFOLLOW.length}]`;

      const status = await instagram.unfollowUser(handle);
      results.push({ username: handle, status });
      console.log(`${progress} @${handle} → ${status}`);

      if (i < ACCOUNTS_TO_UNFOLLOW.length - 1) {
    ...
    expect(results.length).toBe(ACCOUNTS_TO_UNFOLLOW.length);
